...
const {
  bridgeCall, hangupCall, acceptCall, startRecording, stopRecording,
} = require('./voiceapi');
...
function  recordingStop() {
    logger.info(`[${call.voice_id}] stop the recording`);
    stopRecording(call.voice_id, () => {});
}
...
function voiceEventHandler(voiceEvent) {
  if(voiceEvent.state != undefined) {
	if (voiceEvent.state === 'incomingcall') {
		...
		//bridgeCall(call.voice_id, call.to, redirect_number, () => {});
		logger.info(eventMsg);
		sseMsg.push(eventMsg);
	} else if (voiceEvent.state && voiceEvent.state === 'bridged') {
		const eventMsg = `[${call.voice_id}] Call is bridged`;
		logger.info(eventMsg);
		sseMsg.push(eventMsg);
		startRecording(call.voice_id , 'anand', () => {});
		setTimeout(recordingStop, 25000);
